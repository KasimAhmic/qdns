...
namespace dns {
  ...
  struct request {
    ...
    static dns::request from(const std::span<const std::byte> &data) {
      ...
      const dns::question question = dns::question::from(std::span(data.data() + dns::HEADER_SIZE,
                                                                   dns::REQUEST_SIZE - dns::HEADER_SIZE));

      return {header, question, dns::answer::from(header, question)};
    }

    [[nodiscard]] std::string to_string() const {
      return std::format("Header: [ {} ] | Question: [ {} ]", this->header.to_string(), this->question.to_string());
    }
  };
} // namespace dns
